...
def generate_grammar(grammar_define):
    ...
    for line in grammar_define.split('\n'):
        ...
        rules = rules.split('|')
        grammar_pattern[key] = rules
        grammar_pattern[key] = [r.split() for r in rules]

    grammar_pattern[ROOT] = grammar_pattern[default]
...
if __name__ == '__main__':
    ...
    while sample_num > 0:
        print(generate(generate_grammar(human)))
        print(generate(generate_grammar(host)))
        sample_num -= 1
